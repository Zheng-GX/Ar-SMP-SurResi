...
% kb=@(H)C1*87./H.^2;
High=[50 100 150];
Gradient=linspace(0.1,2,20)*10^-3;
r0=15;
%%
for iH=1:length(High)
    ...
    for iG=1:length(Gradient)
        ...
        mx=mean(x);
        co= polyfit(tspan, mx, 1);
        vdx_fit(iH,iG)=co(1);
        vdx(iH,iG)=mean(v_u(:,end));
    end
end
% 
 timeStr = datestr(now, 'yyyymmdd_HHMMSS');
 stream = RandStream('mt19937ar', 'Seed', 'shuffle');
 randSuffix = sprintf('%04d', randi(stream, 9999, 1));
 save(['a3_G_vd_r',num2str(r0),'_', timeStr, '_', randSuffix, '.mat'], 'Gradient','vdx','High','vdx_fit','kb');
